...
function dpOpen(pts: Float64Array, idx: Int32Array, a: number, b: number, tol2: number, keep: Uint8Array): void {
  const stack = [a, b];
  keep[idx[a]] = 1;
  keep[idx[b]] = 1;
  while (stack.length) {
...
export function simplifyLoop(pts: Float64Array, tol: number): Float64Array {
  ...
  let far = 1, farD = -1;
  for (let i = 1; i < n; i++) {
    const dx = pts[i * 2] - x0, dy = pts[i * 2 + 1] - y0;
    const d = dx * dx + dy * dy;
  ...
  if (count < 3) {
    ...
    let third = -1, thirdD = -1;
    const fx = pts[far * 2], fy = pts[far * 2 + 1];
    for (let i = 1; i < n; i++) {
      if (i === far) continue;
      const d = segDist2(pts[i * 2], pts[i * 2 + 1], x0, y0, fx, fy);
      if (d > thirdD) { thirdD = d; third = i; }
    }
    keep[third] = 1;
    count = 3;
  }
  ...
  if (count > 3) {
    ...
    for (let i = prev + 1; ok && i < n; i++) {
      ok = segDist2(pts[i * 2], pts[i * 2 + 1], pts[prev * 2], pts[prev * 2 + 1], pts[next * 2], pts[next * 2 + 1]) <= tol2;
    }
    for (let i = 1; ok && i < next; i++) {
      ok = segDist2(pts[i * 2], pts[i * 2 + 1], pts[prev * 2], pts[prev * 2 + 1], pts[next * 2], pts[next * 2 + 1]) <= tol2;
    }
    if (ok) { keep[0] = 0; count--; }
  }
  const out = new Float64Array(count * 2);
  for (let i = 0, o = 0; i < n; i++) {
    if (keep[i]) { out[o++] = pts[i * 2]; out[o++] = pts[i * 2 + 1]; }
  }
  return out;
}
